            ...
            >
                ...
                <div className="space-y-2 max-h-fit rounded-xl md:rounded-2xl lg:rounded-4xl flex-grow">
                    {faculty.programs.map((program, idx) => (
                        <p key={idx} className="bg-gray-100 border-2 border-white p-4 rounded-lg lg:rounded-xl md:rounded-3xl text-xs md:text-sm text-gray-700 hover:bg-primary cursor-pointer transition duration-500">
                            {program}
                        </p>
                    ))}
                </div>
