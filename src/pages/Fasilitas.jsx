            ...
            <div className="relative p-4 md:px-10 lg:px-12 space-y-12 md:space-y-16 lg:space-y-24">
                <div className={"space-y-4 md:space-y-6 lg:space-y-10"}>
                    ...
                    <div className="flex justify-between items-center gap-4">
                        <div className="w-[50%]">
                            <h2 className="text-4xl md:text-6xl lg:text-7xl font-bold whitespace-none break-words text-gray-500">Fasilitas</h2>
                        </div>
                        <div className="w-[50%]">
                ...
                <div className={"space-y-4 md:space-y-6 lg:space-y-10"}>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-2 gap-4 md:gap-6 lg:gap-10">
                        {gedungKampus?.map((gedung, index) => (
                            <GedungCard key={index} title={gedung.title} text={gedung.text} mapUrl={gedung.mapUrl} image={gedung.image} />
                        ))}
                    </div>
                </div>
                    ...
                    <div className="w-full h-full grid grid-cols-1 lg:grid-cols-3 gap-4">
                        <div className="w-full h-full grid grid-cols-1 lg:grid-lols-2 gap-4 ">
                            {bento1?.map((item, index) => (
                                <FasilitasCard key={index} image={item.image} title={item.title} description={item.description} height="h-52 md:h-64 lg:h-80" />
                            ))}
                        </div>
                        <div className="w-full h-full">
                            {bento2?.map((item, index) => (
                                <FasilitasCard key={index} image={item.image} title={item.title} description={item.description} height="h-52 md:h-64 lg:h-full"/>
                            ))}
                        </div>
                        <div className="w-full h-full grid grid-cols-1 lg:grid-lols-2 gap-4">
