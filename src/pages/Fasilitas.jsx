...
const Fasilitas = () => {
  ...
  const [bento1Memo, bento2Memo, bento3Memo] = useMemo(() => {
    const sliceBento1 = fasilitas?.slice(0, 2) || [];
    const sliceBento2 = fasilitas?.slice(2, 3) || [];
    const sliceBento3 = fasilitas?.slice(3, 5) || [];
    return [sliceBento1, sliceBento2, sliceBento3];
  }, [fasilitas]);
    ...
    const fetchFasilitas = async () => {
      try {
        ...
        if (isMounted) {
          ...
          setBento1(response?.slice(0, 2) || []);
          setBento2(response?.slice(2, 3) || []);
          setBento3(response?.slice(3, 5) || []);
        }
      } catch (error) {
        console.error(error);
      }
