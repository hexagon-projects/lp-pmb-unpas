    ...
    return (
        <MotionWrapper className={'w-full flex flex-col gap-4 md:gap-6 lg:gap-10 px-5 md:px-10 lg:px-15'}>
            ...
            <div className="w-full flex flex-col-reverse lg:flex-row-reverse md:justify-evenly items-center gap-4 md:gap-6 lg:gap-8">
                ...
                <div className="hidden lg:flex w-full h-full lg:max-w-1/2 flex-col items-center justify-center gap-4 mt-4 md:mt-0">
                    <Swiper
                    ...
                    >
                        {organisasi1.map((item, index) => (
                            <SwiperSlide
                                key={item.id}
                                className={index % 2 === 0 ? '!w-[63%]' : '!w-[35%]'}
                                style={{ width: index % 2 === 0 ? '63%' : '35%' }}
                            >
                                <div className={`relative w-full h-full rounded-xl md:rounded-2xl lg:rounded-4xl overflow-hidden`}>
                                    <img
                                        ...
                                        alt={item.title}
                                        className="w-full h-[25vh] lg:h-[34vh] xl:h-[30vh] object-cover object-top"
                                        loading="lazy"
                                        decoding="async"
                                    />
                    ...
                    >
                        {organisasi2.map((item, index) => (
                            <SwiperSlide key={item.id}
                                className={index % 2 === 0 ? '!w-[35%]' : '!w-[63%]'}
                                style={{ width: index % 2 === 0 ? '35%' : '63%' }}
                            >
                                <div className="relative w-full h-full rounded-xl md:rounded-2xl lg:rounded-4xl overflow-hidden">
                                    <img
                                        src={`${imageURL}/organisasi/${item.image}`}
                            ...
                            <SwiperSlide key={item.id}>
                                <div className={`w-full relative h-full rounded-xl md:rounded-2xl lg:rounded-4xl overflow-hidden`}>
                                    <img
                                        src={`${imageURL}/organisasi/${item.image}`}
                                        alt={item.title}
                                        className="w-full h-[25vh] md:h-[35vh] lg:h-[34vh] xl:h-[30vh] rounded-xl md:rounded-2xl lg:rounded-4xl object-cover object-top"
                                        loading="lazy"
                                        decoding="async"
                                    />
                                    <div className="absolute inset-0 bg-gradient-to-t from-[#D3C61D]/100 via-[#C5B91B40]/25 to-transparent flex items-end p-4">
