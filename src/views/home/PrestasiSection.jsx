    ...
    return (
        <MotionWrapper className={'w-full flex flex-col gap-4 md:gap-6 lg:gap-10 p-4 md:px-10 lg:px-12'}>
            <div className="text-center">
                <Text text={'Universitas Pasundan'} color="text-red-500" weight={'font-bold'}/>
            ...
            <div className="w-full flex flex-col-reverse md:flex-row-reverse md:justify-evenly items-center gap-4">
                ...
                <div className="w-full md:max-w-1/2 flex flex-col">
                    ...
                    >
                        {prestasi.map((item) => (
                            <SwiperSlide key={item.id}>
                                <div className="w-full rounded-lg md:rounded-2xl lg:rounded-4xl flex flex-col items-center text-center">
                                    <img
                                        src={`${imageURL}/prestasis/${item.image}`}
                                        alt={item.title}
                                        className="w-full h-48 md:h-56 lg:h-64 object-cover rounded-lg md:rounded-2xl lg:rounded-4xl mb-4 shadow-lg drop-shadow-[0px_20px_40px_rgba(254, 242, 81, 0.5)]"
                                    />
                                </div>
                            </SwiperSlide>
                        ))}
                    ...
                    >
                        {prestasi.map((item) => (
                            <SwiperSlide key={item.id}>
                                <div className="w-full rounded-lg md:rounded-2xl lg:rounded-4xl hidden md:flex flex-col items-center text-center">
                                    <img
                                        src={`${imageURL}/prestasis/${item.image}`}
                                        alt={item.title}
                                        className="w-full h-48 md:h-56 lg:h-64 object-cover rounded-lg md:rounded-2xl lg:rounded-4xl mb-4 shadow-lg drop-shadow-[0px_20px_40px_rgba(254, 242, 81, 0.5)]"
