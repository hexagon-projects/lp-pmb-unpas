...
const ArticleCard = ({ data, border, grid = 'grid grid-cols-1 md:grid-cols-4', hideText = false }) => {
  ...
  return (
    <div className={`${grid} gap-2 md:gap-[32px] lg:gap-8 md:pb-8`}>
        ...
        >
          <div className="h-[30vh] md:h-full flex md:flex-col justify-between items-start gap-4">
            <div className="md:p-4 w-1/2 h-full md:w-full">
              <img src={`${imageURL}/posts/${item?.image}`} alt={item?.title} className="w-full h-full object-cover rounded-lg md:rounded-xl lg:rounded-2xl" />
            </div>
            <div className="w-1/2 h-full md:w-full bg-white px-0 md:px-4 md:rounded-b-md flex flex-col justify-between">
              ...
              <div className="md:hidden md:px-4 flex justify-between items-center">
                <motion.div className="flex items-center gap-2 text-black font-bold text-xs md:text-sm cursor-pointer" initial="hidden" whileHover="visible" transition={{ duration: 0.3 }}>
                ...
                <p className="text-gray-700 font-semibold text-xs md:text-xs hidden">{formatDate(item?.pub_date)}</p>
              </div>
            </div>
          </div>
          <div className="hidden md:px-4 md:pt-4 md:flex justify-between items-center pb-2 md:pb-4">
            ...
            <p className="text-gray-700 font-semibold text-xs md:text-xs">{formatDate(item?.pub_date)}</p>
          </div>
        </motion.div>
      ))}
    </div>
