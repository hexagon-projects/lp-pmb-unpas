...
const AgendaCard = ({ data }) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {data.map((item) => (
                <div key={item.id} className="cursor-pointer flex flex-col justify-between p-4 bg-white rounded-md shadow-md space-y-3">
                    <div className="space-y-3">
                        <img src={`${imageURL}/agendas/${item.image}`} alt={item.title} className="w-full h-48 object-cover rounded-md" />
