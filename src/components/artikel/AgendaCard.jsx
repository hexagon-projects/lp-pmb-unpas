...
const AgendaCard = ({ data }) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {data.map((item) => (
                <div key={item.id} className="cursor-pointer flex flex-col justify-between p-4 bg-white rounded-xl md:rounded-2xl lg:rounded-4xl shadow-black/5 shadow-xl drop-shadow-[0px_20px_40px_rgba(254, 242, 81, 0.5)] space-y-3">
                    <div className="space-y-3">
                        <img src={`${imageURL}/agendas/${item.image}`} alt={item.title} className="w-full h-48 object-cover rounded-xl md:rounded-2xl lg:rounded-4xl" />
