...
const HighlightCard = ({ title, text, image }) => {
    ...
    return (
        <div 
            ...
            {/* tampak depan */}
            <div className="flex justify-center items-center text-start gap-3">
            <img src={`${imageURL}/unggulans/${image}`} alt={image} className="w-20 h-20 object-cover rounded-full" />
            <ArticleTitle 
                    color={`transition ${
                        isActive ? 'text-white' : 'text-gray-900 group-hover:text-white'
                    }`} 
